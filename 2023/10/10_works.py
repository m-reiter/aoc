...
class Tile():
  ...
  def connect_from(self, other):
    if self.is_pipe and other.position in self.connections:
      self._from = other
    else:
      raise ValueError
  ...
  def __str__(self):
    ...
    for y in range(self.dimensions.y):
      ...
      for x in range(self.dimensions.x):
        ...
        if Maze.loop_only_printing and not tile.in_loop:
        ...
        line.append(char)
      #lines.append("".join(self[P(x,y)].kind for x in range(self.dimensions.x)))
      #lines.append("".join(self[P(x,y)].kind if self[P(x,y)].in_loop else GROUND for x in range(self.dimensions.x)))
      #lines.append("".join(self[P(x,y)].kind if self[P(x,y)].in_loop else "O" if self[P(x,y)].is_outer else GROUND for x in range(self.dimensions.x)))
...
#    return Tile(GROUND, key)

  def check_loop(self, candidate, origin = None):
    if origin is None:
  ...
  def find_loop(self):
    ...
    for kind, connections in PIPES.items():
      if connections == start_connections:
        ...
        candidate.connect_from(start_pipe)
        endpoint.connect_to(start_pipe)
        start_pipe.connect_from(endpoint)
