...
class Hailstone:
  ...
  def collides_in_xy(self, other):
    ...
    """
    #if sum(x.is_vertical for x in (self, other)) == 1:
    #  return True
    if self.m == other.m:
  ...
  def will_collide_in_3d(self, other):
    collision = self.collides_in_xy(other)
    ...
    return t1 == t2 and self.position(t1) == other.position(t1)
...
def part2(hailstones):
  ...
  to_check = hailstones[1:4]

  while True:
    t2 += 1
    print(t2)
    for t1 in range(1, t2):
      #for first, second in permutations(to_check, 2):
      for first, second in permutations(hailstones, 2):
        if first.position(t2) == second.position(t2):
          print("*** Collision at {} ns!!!".format(t2))
        direction = second.position(t2) - first.position(t1)

        dt = t2 - t1

        if any(v % dt != 0 for v in direction):
          continue

        velocity = direction / dt

        origin = first.position(t1) - t1 * velocity

        if t1 == 1 and t2 == 3:
          print(first.position(t1), second.position(t2), origin, direction, velocity)

        candidate = Hailstone(origin, velocity)

        if all(candidate.will_collide_in_3d(hailstone) for hailstone in hailstones):
          return candidate
