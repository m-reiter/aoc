...
def read_monkeys():
  ...
  for monkey,description in zip(monkeys,data):
    for line in description:
      if MONKEY.match(line):
        ...
        divisor = int(TEST.match(line).group(1))
  ...
  Monkey.divisor = reduce(mul,divisors)
...
def part2(monkeys):
  tmp = run_part(monkeys,number_of_rounds=10000-20,show=1000,cool_down=False)
  print_monkeys(monkeys)
  return tmp
